...
class SearchSolution(Base):
    ...
    def set_base_from_pickle(self):
        ...
        self.reg_matrix = [np.float64] * len(data['reg'])
        self.ids = {} 
        ...
        index_time_params = {'M' : 186, 'indexThreadQty': 16, 'efConstruction': 142, 'post' : 2}
        self.index.createIndex(index_time_params, print_progress=True)
        #self.index.saveIndex('index.bin', save_data=True)

        self.pass_dict = data['pass'] # index -> [vec1, vec2, vec3, ...]
    ...
    def cal_base_speed(self, base_speed_path='./base_speed.pickle') -> float:
        ...
        for i, tup in enumerate(tqdm(self.pass_dict.items(), total=samples)):
            ...
            for q  in passes:
                t0 = time.time()
                c_output = self.search(query=q) # [i1, i2, ..]
                t1 = time.time()
                T_base += (t1 - t0)
    ...
    def search(self, query: np.array) -> List[Tuple]:
        ...
        '''
        #print(query.shape, " ", type(query[0]))
        #self.index.loadIndex('index.bin', load_data=True)
        return self.index.knnQuery(query, k = 566)
